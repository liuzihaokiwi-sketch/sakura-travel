...
> 数据源: [sources/japan/hotels.md](../sources/japan/hotels.md)
> 定位: 编辑型酒店推荐库，不是订房系统
...
按样本量决定取法(见 master-guide 第六章): >=15 用百分位, 6-14 用 fixed N, <6 仅收明确代表项。
同一决策位(area x hotel_type x price_level)最多保留 3 家: 首推/稳妥替代/机动备选。

---

## 二、数据源

酒店数据源角色**按 hotel_type 不同而不同**，不是统一优先级。

**完整数据源目录、三轴分配表、搜索词、访问方式**见 [sources/japan/hotels.md](../sources/japan/hotels.md)。
