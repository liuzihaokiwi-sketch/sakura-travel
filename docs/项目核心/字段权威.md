...
- 装配引擎改由**三信号推断**节奏：(1) 目录位置（`special_dates/` / `special_events/` / `niche_spots/` + `night.json` / 两日变体编号）(2) slots 第一段时间（早于 08:00 视作不平移）(3) 方案层白名单（[docs/写作规范.md §4.1/§4.2](../../japan/kansai/docs/写作规范.md)）

**节奏归属由方案层白名单决定，模板不自报。** [docs/写作规范.md](../../japan/kansai/docs/写作规范.md) §4.1 / §4.2 两张表是唯一权威源。
...
**迁移方向**：
- `pace_type` / `pace_type_sub` → [docs/写作规范.md §4.1/§4.2](../../japan/kansai/docs/写作规范.md) 白名单·方案作者读此两表决定归属·模板无需自报
...
- `exclusive_with` / `min_days` / `selectable_tag` / 打分三件套 → 方案层 [docs/写作规范.md](../../japan/kansai/docs/写作规范.md)
...
| 信号 | 判据 | 推断 |
|---|---|---|
| **方案层白名单**（权威源） | [docs/写作规范.md §4.1](../../japan/kansai/docs/写作规范.md) 列的 5 个模板 | `fixed_early` |
| **方案层白名单**（权威源） | [docs/写作规范.md §4.2](../../japan/kansai/docs/写作规范.md) 列的 7 个模板 | `deep_stay`（附带 `onsen` / `deep_local` 子类） |
...
**具体装配行为**见 [docs/装配引擎.md](../../japan/kansai/docs/装配引擎.md)。
...
**装配引擎**：`scripts/assemble_schedule.py`·详见 [docs/装配引擎.md](../../japan/kansai/docs/装配引擎.md)。
...
- 酒店池 → `japan/kansai/hotels/{city}/{area}.json`（D48 拆分·对齐 restaurants/stops 结构）
...
| 原花名册字段 | 新归属 |
|---|---|
| 打分（core_experience / audience_bonus / execution_risk）| **方案层** [docs/写作规范.md](../../japan/kansai/docs/写作规范.md) §五（候选列表顺序 = 优先级，不用数字公式） |
| min_days / day_type / exclusive_with | **方案层**（方案作者写方案时保证跨日互斥 / 最少天数） |
| 关西 7 条化学反应 | **方案层** [docs/写作规范.md §三](../../japan/kansai/docs/写作规范.md) |
...
| fixed_early / deep_stay 候选总库 | **方案层** [docs/写作规范.md §四](../../japan/kansai/docs/写作规范.md) |
