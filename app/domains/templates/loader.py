...
只读、带缓存。规则来源只有 JSON（policy.json / days/ 目录 / seasonal_events.json），
不读 brief.md 和 docs/intake/ 下的设计文档（那些是给人看的文档）。

Day 模板支持两种文件格式（优先多文件）：
  - 多文件：content/<circle>/<city>/days/_meta.json + 每个模板一个 .json
  - 单文件（向后兼容）：content/<circle>/<city>/days.json
"""
...
class TemplateLoader:
    ...
    def load_city_days(self, city: str) -> dict[str, Any]:
        """Day 模板库（含装配元数据）。优先读 days/ 目录，fallback 读 days.json。"""
        days_dir = self.base / city / "days"
        if days_dir.is_dir():
            return self._load_city_days_dir(days_dir)
    ...
    def list_cities(self) -> list[str]:
        """列出所有有 days/ 目录或 days.json 的城市。"""
        cities = []
        for p in self.base.iterdir():
            if p.is_dir() and ((p / "days").is_dir() or (p / "days.json").exists()):
    ...
    def _load_city_days_dir(self, days_dir: Path) -> dict[str, Any]:
        """从 days/ 目录加载：_meta.json + 每个模板文件合并。"""
        meta = self._read_json(days_dir / "_meta.json")
        templates = []
        for f in sorted(days_dir.glob("*.json")):
            if f.name.startswith("_"):
                continue
            data = self._read_json(f)
            # 支持单模板文件（有 template_id）和多模板文件（有 templates 数组）
            if "template_id" in data:
                templates.append(data)
            elif "templates" in data and isinstance(data["templates"], list):
                templates.extend(data["templates"])
        result = dict(meta)
        result["day_templates"] = templates
        return result
