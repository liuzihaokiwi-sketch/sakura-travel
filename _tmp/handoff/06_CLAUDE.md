...
```
4屏表单 → TemplatePlanner(纯规则) → plan_preview → 用户纠偏 → plan_confirm → BudgetCalculator → budget_confirm → 手账本渲染
```

关键文件：
- `app/api/trips_v2.py` — 7个API端点 `/v2/trips`
- `app/domains/planning_v2/template_planner.py` — 纯规则方案生成
- `app/domains/planning_v2/budget_calculator.py` — 预算计算（升降档混搭）
- `app/domains/templates/loader.py` — 模板数据读取
- `content/kansai/` — 模板数据目录（brief/policy/days/restaurants/hotels/shops）

设计文档：
- `content/kansai/brief.md` — 产品原则（唯一原则文档）
- `content/kansai/policy.json` — 硬约束
- `content/kansai/form_design.md` — 表单与确认页设计

## 16 步管线架构（Legacy，代码仍存在）

> 新流程用 TemplatePlanner，16步管线代码保留但不再是主链路。
...
| 文件 | 用途 | 代码消费方 |
|------|------|-----------|
| `content/kansai/policy.json` | 硬约束、城市分配、预算规则、校验清单 | TemplatePlanner |
| `content/kansai/tag_vocab.json` | 受控标签词表 | 数据校验 |
| `content/kansai/{city}/days.json` | Day模板库 + 装配元数据 | TemplatePlanner |
| `content/kansai/{city}/restaurants.json` | 餐厅池（标签驱动） | 手账本填充 |
| `content/kansai/{city}/hotels.json` | 酒店池 | 手账本填充 |
| `content/kansai/{city}/shops.json` | 店铺池 | 手账本填充 |
| `content/kansai/live_facts/` | 票价/营业时间/预约 | 手账本运营层 |
| `app/api/trips_v2.py` | 7个API端点 | 前端表单 |
| `app/domains/planning_v2/template_planner.py` | 方案生成引擎 | trips_v2 API |
| `app/domains/planning_v2/budget_calculator.py` | 预算计算 | trips_v2 API |
